# Challenger v2: based on the validator-compatible ai-toolkit image.
FROM diagonalge/ai-toolkit:latest

WORKDIR /app/ai-toolkit

# Pin the toolkit source used by the current G.O.D image trainer.  Reinstalling
# requirements after checkout prevents the mutable base image from silently
# changing Python dependencies beneath the pinned code.
...
COPY ops/docker/scripts/image_recipe /usr/local/bin/image_recipe
